...
int main(int argc, char** argv)
{
    ...
    bodies.push_back( Body( 0, 0, 1000,    0, 0 , 0,   blue, trail_length ));

    for(int i =0; i < 2; i++)
    {
        particles.push_back( Body( rand()%500 - 250, rand()%500 - 250, .001,    0, rand()%10 - 5 , 0,   red, trail_length ));
    }
...
int running = 1;    
    ...
    while(running){
    	...
    	for(i=0; i<particles.size()-1; i++){
    		...
    		bodies[i] = current_body;
    	}


        if(nosmall==1 && ts_smaller)
        {
            timestep=old_ts;
        }
